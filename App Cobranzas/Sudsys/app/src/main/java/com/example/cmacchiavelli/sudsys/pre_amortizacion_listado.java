...
public class pre_amortizacion_listado extends AppCompatActivity {
    ...
    String parametro_id_empleado, parametro_id_sucursal, parametro_tipo, parametro_id_cliente, parametro_id_recibo, mensaje,
            ...
            parametro_ci_nit_cliente, parametro_tipo_cliente;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_pre_amortizacion_listado);

        //incluimos el tool_bar
        toolbar=(Toolbar)findViewById(R.id.tool_bar);
        toolbar.setTitleTextColor(Color.parseColor("#FFFFFF"));
        toolbar.setTitle("Pre Amortización Cliente2");
        ...
        pre_amortizacion_listado.SegundoPlano tarea = new pre_amortizacion_listado.SegundoPlano();
        tarea.execute();
        ...
        pre_amortizacion_listado.TercerPlano tarea2 = new pre_amortizacion_listado.TercerPlano();
        tarea2.execute();
    }
    ...
    private void convertir(){
        ...
        try {
            ...
            Request.addProperty("numero_liquidacion", "");
            ...
            for (int i = 0; i< SoapArrayNivel2.getPropertyCount(); i++)
            {
                ...
                data3.add(new CuotasPreCliente(ClientesArray[i][0],  ClientesArray[i][3], R.mipmap.ic_launcher_round, ClientesArray[i][17], ClientesArray[i][15], ClientesArray[i][3], ClientesArray[i][2], parametro_id_cliente, parametro_id_recibo, ClientesArray[i][1], ClientesArray[i][16].replace(" ",""),  ClientesArray[i][5],  ClientesArray[i][18],  ClientesArray[i][19],  ClientesArray[i][20], parametro_id_sucursal,   ClientesArray[i][9], parametro_id_empleado, "1234"));
